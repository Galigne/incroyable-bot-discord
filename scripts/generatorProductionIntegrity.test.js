...
test('adventure composition reuses helper vocabularies and targeted event references', () => {
	...
	for (const locale of ['en', 'fr']) {
		...
		for (const [entryId, reference] of [
			['every_flame_in_the_area_turns_green_and', 'site.generator.name'],
			['a_terrified_courier_mistakes_a_party_member_for', 'faction.name'],
			['a_wounded_monster_crashes_into_the_scene_fleeing', 'monster.name'],
			['a_festival_starts_unexpectedly_its_central_competition_recreates', 'ability.name'],
			['a_child_calmly_asks_the_party_to_remove', 'building.name'],
			...
			['a_bridge_collapses_after_the_party_crosses_separating', 'settlement.name'],
			['a_magical_market_appears_between_two_trees_and', 'loot.generator.name'],
			['a_dying_raven_delivers_a_signet_ring_and', 'settlement.name'],
			['the_nearest_river_runs_backward_exposing_a_wreck', 'curio.name'],
			[
				'a_famous_adventurer_publicly_credits_the_party_for',
				'background:adventurer.generator.name',
			],
			['a_wandering_judge_declares_an_improvised_trial_over', 'loot:curio.generator.name'],
			['a_harmless_animal_begins_speaking_with_the_voice', 'background.generator.name'],
			['the_moon_briefly_shows_a_second_face_after', 'settlement.name'],
			['a_bell_rings_underground_local_elders_insist_there', 'religion.name'],
			['sudden_tremor', 'building.name'],
			...
			['shared_memory', 'site.generator.name'],
			['vanishing_landmark', 'building.name'],
			['duplicated_object', 'loot.generator.name'],
			['refugees_arrive', 'settlement.name'],
			['refugees_arrive', 'monster.name'],
